...
public class PluginApkInfo implements Serializable {
	...
	/**
	 * 通过Intent查找到对应的receivers
	 * 
	 * @param intent
	 * @return
	 */
	public List<PluginReceiver> queryReceiverByIntent(Intent intent) {
		if (intent == null) {
			return null;
		}
		String tag = "PluginApkInfo:queryReceiverByIntent";
		long time = System.currentTimeMillis();
		List<PluginReceiver> returnReceivers = matchComponentName(
				this.receivers, intent);
		// 要返回的信息集合
		PluginLog.d(tag,
				"search receiver fail, time is " + PluginLog.countTime(time));
	 ...
	 * @return
	 */
	private <T> List<T> matchComponentName(
			List<? extends BaseARSPluginComponent> searchList, Intent intent) {
		ArrayList<BaseARSPluginComponent> returnReceivers = new ArrayList<BaseARSPluginComponent>();
		for (BaseARSPluginComponent activity : searchList) {
			...
			for (PluginIntentFilter2 intentFilter : arrayList) {
				...
				if (r) {
					...
					returnReceivers.add(activity);
				}
			}
		}
		return (List<T>) returnReceivers;
	}
	...
	public List<PluginActivity> queryActivityByIntent(Intent intent) {
		...
		// 要返回的信息集合
		List<PluginActivity> returnActivities = matchComponentName(activities,
				intent);
		PluginLog.d(tag,
	 ...
	 * @return
	 */
	public <T> List<T> queryComponetByIntent(Intent intent, String type) {
		if (intent == null || type == null) {
			return null;
		}
		String tag = "PluginApkInfo:queryByIntent";
		long time = System.currentTimeMillis();
		List<? extends BaseARSPluginComponent> searchList = null;
		if ("sevice".equals(type)) {
			searchList = this.services;
		} else if ("activity".equals(type)) {
			searchList = this.activities;
		} else if ("receiver".equals(type)) {
			searchList = this.receivers;
		}
		System.out.println("PluginApkInfo.queryComponetByIntent()searchList"+searchList);
		if (searchList == null || searchList.isEmpty()) {
			return null;
		}
		// 要返回的信息集合
		List<T> returnActivities = matchComponentName(searchList, intent);
		PluginLog.d(tag,
				"search service , time is " + PluginLog.countTime(time));
		return returnActivities;
	 ...
	 * @return
	 */
	public List<PluginService> queryServiceByIntent(Intent intent) {
		if (intent == null) {
			return null;
		}
		String tag = "PluginApkInfo:queryServiceByIntent";
		long time = System.currentTimeMillis();

		// 要返回的信息集合
		List<PluginService> returnActivities = matchComponentName(activities,
				intent);
		PluginLog.d(tag,
				"search service , time is " + PluginLog.countTime(time));
		return returnActivities;
